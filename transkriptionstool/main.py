...
# -*- coding: utf-8 -*-
"""
Transkriptionstool für MP4-Dateien
Erstellt eine einfache GUI zum Hochladen von MP4-Dateien,
Extrahieren des Audios und Transkribieren in eine Word-Datei.
"""
...
class TranskriptionsApp:
    ...
    def create_widgets(self):
        ...
        self.drop_area = ttk.Label(
            upload_frame,
            text="Ziehen Sie eine MP4-Datei hierher oder klicken Sie zum Auswählen",
            relief=tk.SOLID,
            padding=10,
            wraplength=500,
            cursor="hand2"
    ...
    def register_drag_drop(self):
        """Registriert Drag & Drop Funktionalität"""
    ...
    def on_drop_enter(self, event):
        """Ändert das Aussehen beim Drag-Enter"""
        self.drop_area.config(background='#e0e0ff')
    
    def on_drop_leave(self, event):
        """Ändert das Aussehen beim Drag-Leave"""
        self.drop_area.config(background='#f0f0f0')
    ...
    def on_drop(self, event):
        ...
        file_path = event.data.strip('{}')  # Entferne Klammern bei einigen Systemen
        
        # Überprüfen ob es eine MP4-Datei ist
        if file_path.lower().endswith('.mp4'):
            self.file_path = file_path
            self.update_file_info()
            self.transcribe_btn.config(state=tk.NORMAL)
        else:
            messagebox.showerror("Fehler", "Bitte wählen Sie eine MP4-Datei aus.")
